...
coref_file='data/wikievent/coref'

'''
Scorer for argument extraction on ACE & wikievents.
...
def WikiEvaluate(args,predictions):
    ontology_dict = load_ontology(dataset=args.dataset)
    # if args.dataset == 'wikievents' and args.coref and not args.coref_file:
    #     print('coreference file needed for the wikievents dataset.')
    #     raise ValueError
    ...
    split=os.path.basename(args.gold_file).split('.')[0]
    coref_mapping = defaultdict(dict)  # span to canonical entity_id mapping for each doc
    if args.coref:
        if args.dataset == 'wikievents':
            with open(f'data/wikievents/coref/{split}.jsonlines', 'r') as f, open(args.gold_file, 'r') as test_reader:
                ...
                for line in f:
                    ...
                    for entity in doc['entity_mentions']:
                        ...
                        coref_mapping[doc_id][
                            (entity['start'], entity['end'] - 1)] = ent_id  # all indexes are inclusive
    ...
    result_dic= {'metrics':{'P': role_prec*100, 'R': role_rec*100, 'f1': role_f*100}}
    ...
    if args.coref:
        ...
        coref_result_dic={'P':role_prec*100,'R':role_rec*100,'f1':role_f*100}
        result_dic['coref_metrics']=coref_result_dic
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gold-file', type=str,default='data/ace/zs-freq-10/test.oneie.json')
    parser.add_argument('--coref-file', type=str)
    parser.add_argument('--head-only', action='store_true')
    parser.add_argument('--coref', action='store_true')
    parser.add_argument('--dataset',type=str, default='ACE', choices=['ACE', 'wikievents','AIDA'])
