...
class SoftTemplate(Template):
    ...
    def generate_parameters(self) -> None:
        ...
        if self.initialize_from_vocab:
            vocab_ids=list(range(5000))
            random.shuffle(vocab_ids)
            # soft_embeds = self.raw_embedding.weight[:5000].clone().detach()
            soft_embeds =torch.index_select(self.raw_embedding.weight,dim=0,index=torch.tensor(vocab_ids[:self.num_tokens])).clone().detach()
        else:
    ...
    def process_batch(self, batch: Union[Dict, InputFeatures]) -> Union[Dict, InputFeatures]:
        ...
        batch_copied=deepcopy(batch) # for cycle iteration
        batch_copied['input_ids'] = None
        batch_copied['inputs_embeds'] = inputs_embeds
        if 'attention_mask' in batch_copied and self.num_tokens>0:
            am = batch_copied['attention_mask']
            batch_copied['attention_mask'] = torch.cat([torch.ones((batch_size, self.num_tokens), dtype = am.dtype, device=am.device), am], dim=-1)
        return batch_copied
