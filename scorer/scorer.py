...
from scorer.constraints import Constraints #
import scorer.scoring_utils as util #
from typing import List
class Scorer(object):
  def __init__(self,args,outputs:List[dict]=None):
    self.role_string_mapping = {}
    self.roles = set()
    self.gold = self.read_gold_file(args.gold_file)
    if args.reuse_gold_format:
      self.pred = self.read_gold_file(args.pred_file, jsonlines=outputs,confidence=False)
    else:
      self.pred = self.read_preds_file(args.pred_file)
    self.constraints = Constraints(args.constraints_file)
  ...
  def read_gold_file(self,file_path, jsonlines=None, confidence=False):
    """
    ...
    def process_example(json_blob):
      ...
      def span_to_sent(span):
        ...
        try:
            ...
            ipdb.set_trace()
        # if not  (sent_start == sent_end):ipdb.set_trace()
        return sent_start
      ...
      if not (all([arg[0] == evt_trigger_span
                   for arg in gold_evt_links])): ipdb.set_trace()
      return (doc_key, gold_evt_links, evt_trigger_types, span_to_sent)
      
    if jsonlines==None:
        jsonlines = open(file_path, 'r').readlines()
    lines = [process_example(json.loads(line)) if type(line)==str else process_example(line) for line in jsonlines  ]
...
if __name__ == "__main__":
  ...
  parser.add_argument('-g', '--gold_file', type=str,default='./data/RAMS/toy/5/test.json',
                      help='Gold file path')
  parser.add_argument('-p', '--pred_file', type=str,default='./experiments/test_output.jsonl',
