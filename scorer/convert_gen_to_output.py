...
# from scorer import *
from scorer.scoring_utils import find_head, WhitespaceTokenizer, find_arg_span
import spacy
from typing import *
...
def extract_args_from_template(ex, template, ontology_dict,):
    ...
    predicted_args = defaultdict(list) # modified for wikievents
    t_ptr= 0
    p_ptr= 0 
    ...
    while t_ptr < len(template_words) and p_ptr < len(predicted_words):
        if re.match(r'<(arg\d+)>', template_words[t_ptr]):
            ...
            if predicted_words[p_ptr] == '<arg>' or predicted_words[p_ptr] == re.split('\d+',arg_name)[-1]:
                # missing argument
                p_ptr +=1
                t_ptr +=1  
            else:
                arg_start = p_ptr 
                while (p_ptr < len(predicted_words)) and t_ptr+1< len(template_words) and (predicted_words[p_ptr] != template_words[t_ptr+1]):
                    p_ptr+=1
...
def covert_gen2outs(args,predictions,GoldExamples:List[InputExample]):
    ...
    examples = {}
    # for instance in GoldExamples:
    #     ex={}
    #     doc_key=instance.meta['doc_key']
    #     ex['ref_evt_links']=instance.meta['evt_links']
    #     ex['gold_evt_links'] = []
    #     ex['sentences']=instance.meta['sentences']
    #     examples[doc_key] = ex
...
if __name__ == '__main__':
    ...
    covert_gen2outs(args,{"doc_key":"nw_RCfd4becb04d2b8343b001dcce40b66324b3aff8e444a09bd7692777d4",
                          'predicted': "Obama attacked Secretary of State John Kerry using reticence at <arg4> place. in order to take leverage",
                    "gold": "Obama attacked Secretary of State John Kerry using reticence at <arg4> place. in order to take leverage"
                    })
